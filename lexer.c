    ...
    switch (token.token) {
        ...
        case TK_ID:
            printf("<%s, pos {%d} >", 
                ...
                token.token == TK_CONST_FLOAT ? "const_float" : "const_char",
    ...
    while (1) {
        switch (state) {
            ...
            case 4: // ID or reserved word
                if (strcmp(lexeme, "programa") == 0) return create_token(TK_PROGRAMA);
                if (strcmp(lexeme, "if") == 0) return create_token(TK_IF);
                if (strcmp(lexeme, "then") == 0) return create_token(TK_THEN);
                if (strcmp(lexeme, "elseif") == 0) return create_token(TK_ELSEIF);
                if (strcmp(lexeme, "else") == 0) return create_token(TK_ELSE);
                if (strcmp(lexeme, "while") == 0) return create_token(TK_WHILE);
                if (strcmp(lexeme, "do") == 0) return create_token(TK_DO);
                if (strcmp(lexeme, "char") == 0) return create_token(TK_CHAR);
                if (strcmp(lexeme, "int") == 0) return create_token(TK_INT);
                if (strcmp(lexeme, "float") == 0) return create_token(TK_FLOAT);
            ...
            case 23: // after char content
                if (current_char == '\'') {
                    ...
                    printf("Error at line %d, column %d: Expected closing quote for character literal\n",
                           current_pos.line, current_pos.column);
                    return create_token(TK_ERROR);
                }
                break;
